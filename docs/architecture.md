...
abstractions are to be added. Only bug fixes on existing modules, and
new publishers implemented strictly against the existing `Publisher`
interface, are in scope. New functionality beyond that is only added
once real-world usage shows it is actually needed — not speculatively.
...
- Additional publishers beyond PeerTube and Internet Archive (YouTube, Rumble, etc.)
